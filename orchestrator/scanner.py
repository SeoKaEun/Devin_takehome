...
def _build_body(pkg, version, details, manifest):
...
def scan_once(log=print):
    ...
    filed, deferred = 0, 0
    for manifest in config.SCAN_MANIFESTS:
        ...
        log(f"[scan] {manifest}: {len(pins)} pins, "
            f"{len(findings)} package(s) with advisories: "
            + (", ".join(f['package'] for f in findings) or "none"))
        ...
        for f in findings:
            ...
            title = (f"[security] {f['package']} {f['version']} vulnerable "
                     f"({lead}) - auto-detected")
            issue = _create_issue(
                title, _build_body(f["package"], f["version"], details, manifest))
            known += "\n" + title  # dedupe within this pass too
            filed += 1
            log(f"[scan] filed issue #{issue['number']}: {title}")
    if deferred:
        log(f"[scan] burst cap {config.SCAN_MAX_NEW} reached - "
            f"{deferred} finding(s) deferred to next cycle")
    return filed
